(() => {
  ...
  const replace = () => {
    ...
    meta.firstElementChild.insertAdjacentHTML(
      "afterend",
      `<li><time datetime="${modifiedTime}" itemprop="dateModified"><i class="fa fa-clock-o" aria-hidden="true"></i> ${modifiedTime.split("T")[0]}</time></li>`
    );
  };
  replace();
  ...
  window.addEventListener("pjax:complete", () => { loadStatistic("https://blog-qexo.ver.moe"); });
  const getCDNinfo = () => {
